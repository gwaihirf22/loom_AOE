...
#
# This exists because the guesses below can only ever cover the DEFAULT
# library. A Steam library on a second drive lives wherever the player put
# it, and no amount of cleverness here finds it. Parsing Steam's own
# library manifest could, and that scanning lives under tools/ and stays
# there: it is a development concern, and pulling it in would make a
# shipped feature depend on the shape of another program's config file.
# Inside a Flatpak this variable is also how the player points Loom at the
# folder they opened a sandbox hole for - see docs/install-linux.md.
