...
def test_every_shipped_asset_directory_is_installed():
    ...
    # Two exceptions, and paths.py is where both are decided. Named from
    # paths rather than typed, so they follow if either is ever renamed.
    #
    # captures/ is development scratch, which is why paths.installed() sends
    # it to the player's data directory rather than here.
    #
    # stats/ is the player's OWN match history from before it moved out of
    # the source tree - LEGACY_STATS_DIR, which migrate() copies across into
    # STATS_DIR and never writes to again. It only exists in a clone old
    # enough to have played before the move, which is why this test passed
    # everywhere for months and failed the first time the suite ran on the
    # machine that had. Shipping it would put my saved games in the package.
    shipped = sorted(beside_the_code - {paths.CAPTURES_DIR.name,
                                        paths.LEGACY_STATS_DIR.name})
...
def test_the_players_own_match_history_does_not_ship():
    """The exception above, asserted from the other side.

    The test above excludes stats/ from what MUST be installed. Nothing in
    that says it must not be, and the difference matters because the obvious
    way to make that test go green is to add stats/ to the manifest's cp -
    at which point the package carries whoever built it's saved games. That
    is not a size problem like captures/, it is a privacy one, and the fix
    that causes it looks like a one-word fix. So it gets its own test."""
    assert paths.LEGACY_STATS_DIR.name not in installed_directories()
