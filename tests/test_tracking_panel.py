...
def test_an_unread_count_says_so_even_when_the_build_wants_something():
    """The reported fault, and it was this test's old assertion.

    An unread count used to fall back to the TARGET alone, so a player
    with eleven villagers on wood against a build wanting five saw "5" -
    which reads as a wrong number rather than a missing one, and was
    reported as exactly that. The dash is the honest half of the pair.

    The rule was already right in the no-build branch two tests up, with
    a docstring explaining why. It simply had not been carried across.
    """
    assert resource_cell(6, None)[0] == "–/6"
    # And the bare dash still stands where there is no build at all: a
    # lone "–/None" would be worse than the thing it replaced.
    assert resource_cell(None, None)[0] == "–"
    # Nothing was read, so nothing can be off target.
    assert resource_cell(6, None)[1] is False
